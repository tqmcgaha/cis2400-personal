...
// and arbitrary key type (via using a typedef of a void*).
//
// Note that since the key and value types are void*, most of the time your
// key and value must be restricted to some kind of pointer. For example,
// if one person wanted to use a struct as a value, they would instead need
// to store a pointer to a struct as the value type.
//
// To support arbitrary types, we also require that the caller specify several
// functions when the HashTable is allocated. These functions are to carry out
// some necessary functionality that is dependent on the types of the Key and
// Value. These include example:
// - a function that takes in a HTKey_t and returns its hash
// - a function that takes in two HTKey_t's and returns true
//   if they are equal (this is to account for possible Hash Collisions)
//
// It's up to the customer to figure out how to produce these appropriate
// functions, but we provide an implementation of FNV hashing
// to help the customer implement their own hash function.
//
// The user must also supply a function to HashTable_Free that specifies
// how to free each HTKeyValue_t that is still in the HashTable when it
// is freed.
//
...
// that frees the key and payload. The pointed-to function is invoked once for each
// value in the HashTable.
typedef void(*ValueFreeFnPtr)(HTKeyValue_t value);
...
typedef bool(*CollisionFnPtr)(HTKey_t value1, HTKey_t value2);

// Hash Function pointer
// takes in a key type and returns the Hash of the key
typedef HTHash_t(*HashFnPtr)(HTKey_t key);
...
//
// note that this is not suitable as a standalone HashFn we supply when
// creating a HashTable. Instead this is useful for writing your own HashFn
// For example:
// HTHash_t String_Hash(HTKey_t string) {
//    char* str = (char*) string;
//    int len = strlen(str);
//    return FNVHash64( (unsigned char*) str, len);
// }
//
// Arguments:
...
void HashTable_Free(HashTable *table, ValueFreeFnPtr value_free_fn);
