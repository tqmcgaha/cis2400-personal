...
// To hide the implementation of our Deque, we declare the "struct dq"
// structure and its associated typedef here, but we *define* the structure
// in the internal header Deque_priv.h. This lets us define a pointer
// to Deque as a new type while leaving the implementation details
// private to the customer.
typedef struct dq_st Deque;

// DQPayload type definition:
...
typedef void* DQPayload_t;

// When we free our Deque, there may be items still in the list.
// Since we want our deque to be generalizable, we need the user to specify
// how to free the elements that are still in the deque, thus the
// function pointer definition we have below.
typedef void(*DQPayloadFreeFnPtr)(DQPayload_t payload);
...
// - true on success.
bool Deque_Pop_Front(Deque *dq, DQPayload_t *payload_ptr);
...
// - false on failure (eg, the deque is empty).
// - true on success.
bool Deque_Peek_Front(Deque *dq, DQPayload_t *payload_ptr);
...
// - false: on failure (eg, the deque is empty).
// - true: on success.
bool Deque_Pop_Back(Deque *dq, DQPayload_t* payload_ptr);
...
// - false on failure (eg, the deque is empty).
// - true on success.
bool Deque_Peek_Back(Deque *dq, DQPayload_t *payload_ptr);
...
// A Deque iterator.
typedef struct dqiter_st DQIterator;  // same trick to hide implementation.
...
void DQIterator_Get(DQIterator *iter, DQPayload_t *payload);
