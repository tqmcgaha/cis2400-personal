...
static void NoOpFree( [[maybe_unused]] HTValue_t freeme) { }
...
bool HashTable_Insert(HashTable *table,
  ...
  DQIterator* iter = HashTable_find_kv_position(table, newkeyvalue.hash, newkeyvalue.key);
  bool ret_val = false;
  if (iter != NULL) {
    HTKeyValue_t* kv;
...
static void MaybeResize(HashTable *ht) {
  ...
  // record.
  newht = HashTable_Allocate(ht->num_buckets * 9, ht->collision_fn);
