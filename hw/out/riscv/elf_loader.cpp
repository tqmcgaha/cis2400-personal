...
static expected<vector<Elf32_Shdr>, string> read_section_headers(
    FILE* elf_file,
    const Elf32_Ehdr& header);

static expected<Elf32_strtab, string> read_string_table(
    FILE* elf_file,
    const Elf32_Ehdr& header,
    const vector<Elf32_Shdr>& section_headers);

static expected<vector<Instruction>, string> read_text(
    FILE* elf_file,
    const Elf32_Shdr& text_header);

static expected<vector<symbol>, string> read_symtab(
    FILE* elf_file,
    const Elf32_strtab& strtab,
    const Elf32_Shdr& symtab_header);

static expected<vector<relocation>, string> read_rela_text(
    FILE* elf_file,
...
expected<riscv_elf, string> load_riscv32i_elf(const string& elf_fname) {
  ...
  result.header = header_res.value();

  // section headers
  auto section_headers_res = read_section_headers(elf_file, result.header);
  if (!section_headers_res) {
    return unexpected(section_headers_res.error());
  }
  result.section_headers = section_headers_res.value();

  // string table
  auto str_tab_res =
      read_string_table(elf_file, result.header, result.section_headers);
  if (!str_tab_res) {
    return unexpected(str_tab_res.error());
  }
  result.string_table = str_tab_res.value();

  // iterate through section headers
  for (size_t i = 0; i < result.section_headers.size(); ++i) {
    Elf32_Shdr s_hdr = result.section_headers.at(i);

    // get the string equivalent of the section header name field & get the type
    const char* section_name = &result.string_table.contents.at(s_hdr.sh_name);
...
static expected<vector<Instruction>, string> read_text(
  ...
  fread(encodings.data(), sizeof(uint32_t), encodings.size(), elf_file);
