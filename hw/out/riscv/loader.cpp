...
expected<riscv_elf, string> load_riscv32i_elf(const string& elf_fname) {
  ...
  Elf32_Shdr shstrtab_header = section_headers[header.e_shstrndx]; // get the string table
  vector<char> shstrtab(shstrtab_header.sh_size); // allocate a vector of the size of string table 
  fseek(elf_file,shstrtab_header.sh_offset, SEEK_SET ); // go to the table in the elf file 
  fread(shstrtab.data(), shstrtab_header.sh_size, 1, elf_file); // read it into the vector  
//   cout << "Section header string table size: " << shstrtab.size() << endl;
  ...
  for (size_t i = 0; i < section_headers.size(); ++i) {
    ...
    if(strcmp(section_name, ".text") == 0) {
      ...
      vector<uint32_t> instructions(s_hdr.sh_size/4); // create a vector of the number of elemenjts (# bytes of file / 4 bytes per inst)
      fseek(elf_file, s_hdr.sh_offset, SEEK_SET); // move file ptr to start of the .text 
      fread(instructions.data(), sizeof(uint32_t),instructions.size(), elf_file); 
      for(uint32_t instr: instructions){
        Instruction instruction = decode_instruction(instr);
        
        // cout << instruction.mnemonic << " x" << instruction.rd << ", x" << instruction.rs1 << ", x" << instruction.rs2 << ", " << instruction.imm << endl;
